...
type Movies struct {
	...
	Ordering        string `bson:"ordering"`
	Title           string `bson:"title"`
	Region          string `bson:"region"`
	Language        string `bson:"language"`
	Types           string `bson:"types"`
	Attributes      string `bson:"attribute"`
	IsOriginalTitle string `bson:"isoriginaltitle"`
}
...
func dbcount(c *gin.Context) {
	...
	fmt.Println("Movie RegEXP " + regexpattern)
	updatemongo := mgoSession.DB(os.Getenv("MONGO_DATABASE")).C("names")
	// gamesWon, err := updatemongo.Find(bson.M{}).Count()

	updatemongo.Find(bson.M{"titleid": bson.M{"$regex": bson.RegEx{regexpattern, ""}}}).All(&people)
	...
	fmt.Println("Movie Results......")

	// fmt.Println(dta.Directors)
	for _, pps := range people {
		fmt.Println("Title: " + pps.Title)
